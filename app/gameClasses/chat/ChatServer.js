...
var ChatServer = {
	 ...
	 */
	sendToRoom: function (roomId, message, to, fromUsername, from) {
		var self = ige.chat;
    ...
    _onMessageFromClient: function (msg, clientId) {
        ...
        if (!self.emit('messageFromClient', [msg, clientId])) {
            console.log('Message from client: (' + ige.server.playerBag.getPlayerUsernameByClientId(clientId) + ')', msg);
				...
				if (room) {
					if (room.users.indexOf(clientId) > -1) {
						if (msg.text) {
							...
							self.sendToRoom(msg.roomId, msg.text, msg.to, ige.server.playerBag.getPlayerUsernameByClientId(clientId),clientId);
						} else {
